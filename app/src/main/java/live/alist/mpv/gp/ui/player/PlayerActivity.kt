...
import live.alist.mpv.gp.databinding.PlayerLayoutBinding
...
import live.alist.mpv.gp.ui.player.controls.PlayerControls
import live.alist.mpv.gp.ui.theme.MpvexTheme
import live.alist.mpv.gp.utils.history.RecentlyPlayedOps
import live.alist.mpv.gp.utils.media.HttpUtils
import live.alist.mpv.gp.utils.media.SubtitleOps
import live.alist.mpv.gp.utils.storage.FileTypeUtils
import live.alist.mpv.gp.utils.storage.FileFilterUtils
...
class PlayerActivity :
   ...
   */
  private val playlistRepository: live.alist.mpv.gp.database.repository.PlaylistRepository by inject()
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        val siblingFiles = if (launchSource == "video_list" || launchSource == "recently_played_button" || launchSource == "first_video_button") {
          ...
          val videosInFolder =
            live.alist.mpv.gp.repository.MediaFileRepository.getVideosForBuckets(
              context,
              setOf(bucketId)
            )
          val sortedVideos = live.alist.mpv.gp.utils.sort.SortUtils.sortVideos(videosInFolder, videoSortType, videoSortOrder)
          sortedVideos.mapNotNull { video -> files.find { it.absolutePath == video.path } }
        } else {
          files.sortedWith { f1, f2 -> live.alist.mpv.gp.utils.sort.SortUtils.NaturalOrderComparator.DEFAULT.compare(f1.name, f2.name) }
        }
  ...
  companion object {
     ...
     */
    private const val RESULT_INTENT = "live.alist.mpv.gp.ui.player.PlayerActivity.result"
