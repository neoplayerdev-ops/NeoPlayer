package live.alist.mpv.gp.ui.browser.filesystem
...
import live.alist.mpv.gp.utils.media.OpenDocumentTreeContract
...
import live.alist.mpv.gp.BuildConfig
import live.alist.mpv.gp.domain.browser.FileSystemItem
import live.alist.mpv.gp.preferences.BrowserPreferences
import live.alist.mpv.gp.preferences.GesturePreferences
import live.alist.mpv.gp.preferences.MediaLayoutMode
import live.alist.mpv.gp.preferences.preference.collectAsState
import live.alist.mpv.gp.presentation.components.pullrefresh.PullRefreshBox
import live.alist.mpv.gp.ui.browser.cards.FolderCard
import live.alist.mpv.gp.ui.browser.cards.VideoCard
import live.alist.mpv.gp.ui.browser.components.BrowserBottomBar
import live.alist.mpv.gp.ui.browser.components.BrowserTopBar
import live.alist.mpv.gp.ui.browser.dialogs.AddToPlaylistDialog
import live.alist.mpv.gp.ui.browser.dialogs.DeleteConfirmationDialog
import live.alist.mpv.gp.ui.browser.dialogs.FileOperationProgressDialog
import live.alist.mpv.gp.ui.browser.dialogs.FolderPickerDialog
import live.alist.mpv.gp.ui.browser.dialogs.RenameDialog
import live.alist.mpv.gp.ui.browser.dialogs.SortDialog
import live.alist.mpv.gp.ui.browser.dialogs.ViewModeSelector
import live.alist.mpv.gp.ui.browser.dialogs.VisibilityToggle
import live.alist.mpv.gp.ui.browser.selection.rememberSelectionManager
import live.alist.mpv.gp.ui.browser.sheets.PlayLinkSheet
import live.alist.mpv.gp.ui.browser.states.EmptyState
import live.alist.mpv.gp.ui.browser.states.PermissionDeniedState
import live.alist.mpv.gp.ui.utils.LocalBackStack
import live.alist.mpv.gp.utils.media.CopyPasteOps
import live.alist.mpv.gp.utils.media.MediaUtils
import live.alist.mpv.gp.utils.permission.PermissionUtils
 ...
 */
@Serializable
object FileSystemBrowserRootScreen : live.alist.mpv.gp.presentation.Screen {
...
data class FileSystemDirectoryScreen(
  val path: String,
) : live.alist.mpv.gp.presentation.Screen {
  @OptIn(ExperimentalPermissionsApi::class)
  @Composable
  override fun Content() {
...
fun FileSystemBrowserScreen(path: String? = null) {
  ...
  val navigationBarHeight = live.alist.mpv.gp.ui.browser.LocalNavigationBarHeight.current
  ...
  LaunchedEffect(
    ...
    if (isAtRoot) {
      try {
        val mainScreenObj = live.alist.mpv.gp.ui.browser.MainScreen
          ...
          val results = if (isAtRoot) {
            ...
            parentDirectories.forEach { parentPath ->
              try {
                ...
                val parentResults = live.alist.mpv.gp.ui.browser.filesystem.searchRecursively(context, parentPath, searchQuery)
            ...
            items.filterIsInstance<FileSystemItem.Folder>().forEach { storageVolume ->
              try {
                ...
                val rootResults = live.alist.mpv.gp.ui.browser.filesystem.searchRecursively(context, storageVolume.path, searchQuery)
            ...
            val results = live.alist.mpv.gp.ui.browser.filesystem.searchRecursively(context, currentPath, searchQuery)
  ...
  live.alist.mpv.gp.ui.browser.fab.FabScrollHelper.trackScrollForFabVisibility(
    listState = listState,
    gridState = null,
    ...
    Scaffold(
      topBar = {
        if (isSearching) {
          ...
          BrowserTopBar(
            title = if (isAtRoot) {
              stringResource(live.alist.mpv.gp.R.string.app_name)
            } else {
            ...
            onSettingsClick = {
              backstack.add(live.alist.mpv.gp.ui.preferences.PreferencesScreen)
            },
            ...
            onInfoClick = if (videoSelectionManager.isInSelectionMode && !folderSelectionManager.isInSelectionMode) {
              {
                ...
                if (video != null) {
                  val intent = Intent(context, live.alist.mpv.gp.ui.mediainfo.MediaInfoActivity::class.java)
          ...
          FloatingActionButtonMenu(
                ...
                ToggleFloatingActionButton(
                  modifier = Modifier
                    .animateFloatingActionButton(
                      visible = !isInSelectionMode && isFabVisible.value && !live.alist.mpv.gp.ui.browser.MainScreen.getPermissionDeniedState(),
            ...
            FloatingActionButtonMenuItem(
              onClick = {
                isFabExpanded.value = false
                coroutineScope.launch {
                  val recentlyPlayedVideos = live.alist.mpv.gp.utils.history.RecentlyPlayedOps.getRecentlyPlayed(limit = 1)
                    ...
                    if (playlistMode) {
                      ...
                      if (startIndex >= 0) {
                        if (allVideos.size == 1) {
                          ...
                          intent.setClass(context, live.alist.mpv.gp.ui.player.PlayerActivity::class.java)
...
suspend fun searchRecursively(
  ...
  try {
    ...
    val items = live.alist.mpv.gp.repository.MediaFileRepository
...
private suspend fun collectVideosRecursively(
  context: Context,
  folderPath: String,
): List<live.alist.mpv.gp.domain.media.model.Video> {
  val videos = mutableListOf<live.alist.mpv.gp.domain.media.model.Video>()
...
private fun playVideosAsPlaylist(
  context: Context,
  videos: List<live.alist.mpv.gp.domain.media.model.Video>,
) {
...
private fun FileSystemBrowserContent(
  ...
  error: String?,
  isAtRoot: Boolean,
  breadcrumbs: List<live.alist.mpv.gp.domain.browser.PathComponent>,
  ...
  onVideoClick: (live.alist.mpv.gp.domain.media.model.Video) -> Unit,
  onVideoLongClick: (live.alist.mpv.gp.domain.media.model.Video) -> Unit,
  onBreadcrumbClick: (live.alist.mpv.gp.domain.browser.PathComponent) -> Unit,
  folderSelectionManager: live.alist.mpv.gp.ui.browser.selection.SelectionManager<FileSystemItem.Folder, String>,
  videoSelectionManager: live.alist.mpv.gp.ui.browser.selection.SelectionManager<live.alist.mpv.gp.domain.media.model.Video, Long>,
  modifier: Modifier = Modifier,
  isInSelectionMode: Boolean = false,
) {
  val gesturePreferences = koinInject<GesturePreferences>()
  val browserPreferences = koinInject<BrowserPreferences>()
  val thumbnailRepository = koinInject<live.alist.mpv.gp.domain.thumbnail.ThumbnailRepository>()
        ...
        Box(
          ...
          LazyColumn(
            ...
            if (!isAtRoot && breadcrumbs.isNotEmpty()) {
              item {
                live.alist.mpv.gp.ui.browser.filesystem.BreadcrumbNavigation(
                  breadcrumbs = breadcrumbs,
            ...
            items(
              ...
              val folderModel = live.alist.mpv.gp.domain.media.model.VideoFolder(
...
fun FileSystemSortDialog(
  ...
  val browserPreferences = koinInject<BrowserPreferences>()
  val appearancePreferences = koinInject<live.alist.mpv.gp.preferences.AppearancePreferences>()
  ...
  SortDialog(
    ...
    onSortTypeChange = { typeName ->
      live.alist.mpv.gp.preferences.FolderSortType.entries.find { it.displayName == typeName }?.let {
    ...
    onSortOrderChange = { isAsc ->
      browserPreferences.folderSortOrder.set(
        if (isAsc) live.alist.mpv.gp.preferences.SortOrder.Ascending
        else live.alist.mpv.gp.preferences.SortOrder.Descending,
      )
    },
    types = listOf(
      live.alist.mpv.gp.preferences.FolderSortType.Title.displayName,
      live.alist.mpv.gp.preferences.FolderSortType.Date.displayName,
      live.alist.mpv.gp.preferences.FolderSortType.Size.displayName,
    ),
    icons = listOf(
      Icons.Filled.Title,
    ...
    getLabelForType = { type, _ ->
      when (type) {
        live.alist.mpv.gp.preferences.FolderSortType.Title.displayName -> Pair("A-Z", "Z-A")
        live.alist.mpv.gp.preferences.FolderSortType.Date.displayName -> Pair("Oldest", "Newest")
        live.alist.mpv.gp.preferences.FolderSortType.Size.displayName -> Pair("Smallest", "Largest")
        else -> Pair("Asc", "Desc")
      }
    },
    ...
    viewModeSelector = ViewModeSelector(
      ...
      isFirstOptionSelected = folderViewMode == live.alist.mpv.gp.preferences.FolderViewMode.AlbumView,
      onViewModeChange = { isFirstOption ->
        browserPreferences.folderViewMode.set(
          if (isFirstOption) {
            live.alist.mpv.gp.preferences.FolderViewMode.AlbumView
          } else {
            live.alist.mpv.gp.preferences.FolderViewMode.FileManager
          },
        )
      },
    ),
